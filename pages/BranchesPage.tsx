...
const BranchesPage: React.FC = () => {
  ...
  useEffect(() => {
    try {
        ...
        if (saved) setSelectedBranchId(JSON.parse(saved).branchId);
    } catch (e) { console.error(e); }
  }, []);

  useEffect(() => {
    if (mapContainerRef.current && !mapRef.current && branches.length > 0) {
        mapRef.current = L.map(mapContainerRef.current).setView([26.5, 45.5], 5);
        L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png').addTo(mapRef.current);

        branches.forEach(branch => {
            if (branch.lat && branch.lng) {
                const marker = L.marker([branch.lat, branch.lng], { icon: defaultIcon })
                    .addTo(mapRef.current!).bindPopup(`<b>${branch.name}</b>`);
                marker.on('click', () => { isUpdatingFromMap.current = true; handleBranchSelect(branch.id); });
  ...
  useEffect(() => {
    if (selectedBranchId) {
        ...
        if (branch?.lat && branch.lng && mapRef.current) mapRef.current.flyTo([branch.lat, branch.lng], 13);
        Object.keys(markersRef.current).forEach(id => {
            const marker = markersRef.current[id];
            if (marker) {
                marker.setIcon(id === selectedBranchId ? selectedIcon : defaultIcon);
                if (id === selectedBranchId) marker.openPopup();
            }
        });
        if (isUpdatingFromMap.current && branchRefs.current[selectedBranchId]) {
            branchRefs.current[selectedBranchId]?.scrollIntoView({ behavior: 'smooth', block: 'center' });
  ...
  return (
    <div>
      <h1 className="text-4xl font-extrabold mb-2 text-gray-900 dark:text-gray-100">اختر الفرع المناسب لك</h1>
      <p className="text-lg text-gray-500 dark:text-gray-400 mb-8">اختر من القائمة أو تصفح الخريطة.</p>
      ...
      <div className="flex flex-col lg:flex-row-reverse gap-6" style={{ height: 'calc(100vh - 250px)' }}>
          <section aria-labelledby="map-heading" className="w-full lg:w-2/3 h-64 lg:h-full">
              <div className="w-full h-full bg-gray-200 dark:bg-gray-700 rounded-lg shadow-lg z-0" ref={mapContainerRef} />
          </section>
                          ...
                          return (
                              <div key={branch.id} ref={el => { branchRefs.current[branch.id] = el; }}>
                                  <Card 
                                      ...
                                      className={`p-4 text-center flex flex-col justify-between border-2 bg-white dark:bg-gray-800 ${isSelected ? 'border-orange-500 ring-2 ring-orange-500/50' : 'border-transparent'} transition-all cursor-pointer`}
                                  >
                                  {/* Branch Card Content */}
                                  {isSelected && (
                                      <Link to={`/cars/${branch.id}`} className="mt-4 block">
                                          <Button className="w-full">عرض السيارات المتاحة</Button>
                                      </Link>
                                  )}
                                  </Card>
                              </div>
